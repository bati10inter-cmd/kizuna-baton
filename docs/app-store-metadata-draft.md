...
**版数 v0.4 / 2026-07-01 更新（Claude下書き・最終はオーナー）**
...
| スクリーンショット 6.9"(1320×2868) | 🔲 未 | 既存プレビューは 6.7"(1290×2796)。実機/シミュレータキャプチャは APPSTORE-XCODE 後。下§4にキャプション計画 |
| メタデータ文言（本書 §1〜§3） | 🟡 下書き | 本書。最終確定＝オーナー |
...
| フィールド | 上限 | 本書の案 | 文字数 |
|---|---|---|---|
| App名（Name） | 30 | きずなbaton | 9 |
| サブタイトル（Subtitle） | 30 | 家族に「契約」をそっと手渡す | 14 |
...
```
親の契約、把握できていますか？ サブスク・月額・ローンを家族でやさしく整理。
カード番号や暗証番号など機密そのものは保存せず、「どこにある・どう手続きするか」を
そっと残せます。高齢のご家族にも見やすい設計。
```
...
・基本機能は無料でお使いいただけます。さらに便利にお使いいただける有料プラン（月額制・自動更新）もご用意しています。
```
...
```
終活,サブスク管理,契約管理,家計,月額,解約,ローン,エンディングノート,家族,シニア,見守り,定期支払い,財産整理,生前整理
```
...
### 7.1 META 文言・素材
1. 🔲 スクショ 6.9"(1320×2868) を5枚（§4）— シミュレータ取得 or 枠クリエイティブ再生成（APPSTORE-XCODE後が本筋）
2. 🔲 メタデータ文言の**オーナー最終確認**（§1〜§3・freemium表現へ更新済）
