...
### 7.1 META 文言・素材
1. 🔄 スクショ 6.9"(1320×2868) を5枚（§4）— 2026-07-02 シミュレータ取得済だが **β排除前の撮影＝`store-01-home.png` にβバッジ写り込み**（2026-07-10 発見）。**`APP-DEBETA` 実施後に再取得→store版再生成→ASC差替が必須**
...
7. 🔲 **IAP 審査用スクショ**（プラン画面・§6.2-4）＝**両サブスク商品が「メタデータが不足」で止まっている唯一の残り要因**。⛔ `APP-DEBETA` 後（βバッジ写り込み回避＝上記1と同じ順序依存）
