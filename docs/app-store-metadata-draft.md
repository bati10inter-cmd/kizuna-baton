...
> **2026-07-10 実施＝1〜3・グループ表示名 完了。残＝4（審査用スクショ）・5（Small Business）。** 両商品のステータスは審査用スクショ未設定ゆえ **「メタデータが不足」** のまま。
...
4. 🔲 **App内課金の審査用情報**＝レビュー用スクショ（プラン画面）＋必要なら審査メモ。⚠️ **`APP-DEBETA` 実施後に撮る**（現行UIにβバッジが写るため。既存の提出用スクショと同じ順序依存＝TASKS.md APP-DEBETA 行）。**これが両商品の「メタデータが不足」の唯一の残り要因**。
...
>
> ⚠️ **未確定＝EU DSA トレーダーステータス**。アプリ配信を日本のみに設定した**後も**、ASC「ビジネス」画面の赤いバナー「コンテンツを欧州連合（EU）のApp Storeで配信するには、お客様がトレーダーであるかどうかをお知らせいただく必要があります…」は**消えていない**（2026-07-10 実測）。バナーは**アカウント単位**の表示と見られ、文面は「EUで配信するには」という条件付き＝日本限定配信なら非該当と*読める*が、**画面上の確証はない**。提出時に Apple 側がブロックする可能性を否定できないため、**提出前に要再確認**（バナーの「コンプライアンス要件を満たす」リンク先で「トレーダーではない／EU配信しない」旨の申告が要る可能性）。※旧記述「日本限定にしたことで当該要件を回避している」は**推論を断定していたため 2026-07-10 に訂正**。
...
### 7.1 META 文言・素材
1. 🔄 スクショ 6.9"(1320×2868) を5枚（§4）— 2026-07-02 シミュレータ取得済だが **β排除前の撮影＝`store-01-home.png` にβバッジ写り込み**（2026-07-10 発見）。**`APP-DEBETA` は 2026-07-10 実装完了（v113・ローカルコミット・未push）＝再取得が着手可能に**。手順＝`npx cap copy ios`→シミュレータ raw 5枚→`build-appstore-marketing-screenshots.js` で store 版再生成→ASC差替
2. ✅ メタデータ文言の**オーナー最終確認**（§1〜§3）— **2026-07-02 完了**（サブタイトル/プロモ/説明文をv0.8で確定反映・改訂履歴 v0.8 参照）
3. 🔲 App Privacy ラベル回答（§5）を App Store Connect に登録（提出時・IAPはStoreKit純正前提）。**依存なし＝今すぐ着手可**
...
7. 🔲 **IAP 審査用スクショ**（プラン画面・§6.2-4）＝**両サブスク商品が「メタデータが不足」で止まっている唯一の残り要因**。**`APP-DEBETA` は 2026-07-10 実装完了（v113）＝着手可能に**（上記1のスクショ再取得と同一ビルドで撮る）
8. 🔲 **Small Business Program 申請**（手数料30%→15%・§6.2-5）＝owner作業。**依存なし＝今すぐ着手可**
9. 🔲 **EU DSA トレーダーステータス**の要否確定（§6.3 末尾の ⚠️）＝日本のみ配信に設定後も ASC「ビジネス」画面の赤バナーが消えない（2026-07-10 実測）。**提出時ブロックの可能性を否定できないため提出前に要確認**。依存なし＝今すぐ着手可
...
8. ✅ **APPSTORE-IAP**＝StoreKit2 自前プラグイン（竹=¥500/月・¥5,000/年）実装済（v82・`native/ios/App/App/KizunaIAPPlugin.swift`）。**2026-07-10 ASC 側のサブスク商品設定も完了**（商品ID `io.dorize.kizunabaton.take.monthly`/`.yearly`・グループ `take`＝§6.2）。残＝審査用スクショ（§7.1-7）と Sandbox 実機購入検証。
