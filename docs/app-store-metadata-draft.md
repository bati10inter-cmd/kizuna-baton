...
| メタデータ文言（本書 §1〜§3） | 🔴 **文案は確定・ASC反映は一部未実施（2026-07-23 実地検証で判明）** | **2026-07-02 オーナー最終確認済**（App名/サブタイトル/プロモ/説明文）。改訂履歴 v0.8 参照。**⚠️ ただし 2026-07-23 に ASC 実物を目視した結果、`サブタイトル` と `プロモーション用テキスト` の入力欄が空だった**（App名・キーワード・説明文・URL・著作権は入力済）。**本書の「✅確定」は文案の確定であって ASC への入力完了ではない**＝以後この行は「文案確定」と「ASC反映」を分けて記録する。是正は `TASKS.md` MK-ASO-FIX 行。 |
...
| サブタイトル（Subtitle） | 30 | **終活に。サブスクや契約を、家族に手渡す**（2026-07-23 オーナー決定・確定） | 19 |
...
> **📌 サブタイトル確定の経緯（2026-07-23）**：旧案「サブスクや契約を、家族にそっと手渡す」（18字・2026-07-02 確定）は **ASC へ未入力のまま公開されていた**（§0 参照）。是正にあたりオーナーが「そっと」を外した15字案を提示 → **30字枠の半分が空くため、検索関連性でキーワード欄より重いサブタイトル枠へ「終活」を昇格させる案を採用**し「終活に。サブスクや契約を、家族に手渡す」（19字）で確定。「そっと」は検索語として無価値ゆえ削除による検索面の損失はゼロ、かつ本アプリのトーンは「手渡す」で保持される。**キーワード欄からは重複語（終活）を削除して枠を空ける**（§3.2 の再配分は `docs/aso-improvement-plan.md` §2 第3優先）。
...
```
サブスク管理,契約管理,月額,解約,ローン,エンディングノート,シニア,定期支払い,財産整理,生前整理,親,老後,自動更新,支払い管理,身辺整理,引き継ぎ,解約忘れ,月額課金,高齢者
```
（**現行＝91字/100**・**2026-07-23 再配分してASC投入・保存・永続化まで完了**。反映は次バージョン1.5の公開後）

> **📌 2026-07-23 再配分の設計（`docs/aso-improvement-plan.md` §2 第3優先の実施）**
> **削除4語**＝`終活`／`家族`＝**サブタイトルへ昇格**（Apple は名前・サブタイトルとキーワードの重複を非推奨。検索関連性はサブタイトルの方が重いため昇格が正）。`家計`＝家計簿アプリと競合し検索意図がズレる。`見守り`＝高齢者見守りアプリと混同され来訪者の意図とズレる。
> **追加5語**＝`身辺整理`（生前整理の類義・新概念）／`引き継ぎ`（本アプリの中核概念＝バトン）／`解約忘れ`（実在の悩み・「忘れ」が新概念）／`月額課金`（「課金」が新概念）／`高齢者`（「シニア」と意味は近いが表記が異なり日本語では別語としてインデックスされる。ターゲットの実際の検索語）。
> **選定原則**＝(1) 既存語と**形態素レベルで重複しない新概念**のみ追加（日本語では単語分解でインデックスされるため、`サブスク解約`のような複合語は既存の `サブスク管理`＋`解約` でカバー済みと判断し不採用）(2) 価値提案と整合（**節約系〔固定費・お金の管理〕は不採用**＝本アプリの核は「家族に伝える」であり節約ではない。節約目的の流入は価値提案とミスマッチで低継続を招く＝`おひとりさま` を外したのと同じ論理）(3) 既定方針の維持＝`相続`/`遺言` は法務連想回避、`遺品整理` は死後作業で生前準備という思想とズレるため不採用、死の直接表現は使わない。
> **残9字は意図的に空けている**＝Apple Ads の検索人気度データを取得してから埋める方が確度が高いため（`aso-improvement-plan.md` §2 第3優先・MK-ASC-FUNNEL プラン5と連結）。
