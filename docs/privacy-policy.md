...
最終更新: 2026-07-10
版: **v5.1（App Store 公開＝β提供の終了に伴う表記整理 — APP-DEBETA）**。条文中の「β版」を「現行版」へ、将来の提供可能性を示す文脈の「本番版」を「将来の版」へ置換する。**取得する個人情報・利用目的・委託先・提供範囲の変更はなし**（emergencyMode 停止・国外居住家族への不開示・公的書類の不受領はいずれも不変）。利用規約 v3.3.2 と一体で発効し、`PRIVACY_VERSION` v5.0→v5.1・`CONSENT_KEY` `:v27→:v28` の再同意に同乗する（**第9条第0項の置換は「もしもの時」機能の不提供を恒久化するものではなく**、将来提供時に本ポリシー改定＋利用者全員の再同意を経る枠組みは従前のまま維持する）。
（前版 v5.0＝クラウド招待・cross-account 閲覧 対応版〔APP-V2-INVITE-RELEASE・2026-07-08〕。前々版 v4.0＝v2「端末間同期（クラウド保存）」対応。各改訂の詳細は改訂履歴を参照）
...
- **本版（v5.1）は、App Store 公開＝β提供の終了に伴う表記整理版**である。条文中の「β版」を「現行版」へ、将来の提供可能性を示す文脈の「本番版」を「将来の版」へ改めた（取得する個人情報・利用目的・委託先・提供範囲の変更なし）。本注記・附則・改訂履歴に残る「β」への言及は、過去版の経緯の記録である。
...
5. **本人が登録する家族メンバー候補の情報**: 招待先メールアドレス（**本人が招待を発行した場合に、招待先メールアドレスへの到達確認 OTP を送信する目的で取得**）、本人による呼び名候補、関係性候補
...
5. **`kizuna-baton:consent-shown:v27`**: 本利用規約・本ポリシーへの本人同意取得済フラグ（v3.2.1 新規。以降、委託先開示・材料的変更のたびに更新。v4.0 でクラウド保存に伴い `:v25→:v26`。**v5.0 でクラウド招待（招待先メール＝第三者情報の送信・共有関係の保管・cross-account 閲覧）という material change に伴う再同意取得のため `:v26→:v27` に更新**）
