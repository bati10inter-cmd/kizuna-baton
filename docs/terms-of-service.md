...
最終更新: 2026-07-08
版: v3.3.1（クラウド招待解禁に伴う招待上限表記の実装整合 patch＝APP-V2-INVITE-RELEASE / Fable A-3。第6条第5項第2号「同時保留中5件」を実装準拠「保留中の招待と受諾済みの家族メンバーはあわせて5件（5人）」へ訂正。プライバシーポリシー v5.0 と一体で発効。定型約款の軽微な変更として民法第548条の4により整理。受諾時の本人宛メール通知〔第6条第5項第4号〕・メールアドレス確認手続き〔第5条第1項〕は条文を変えず実装追加で履行）
（前版 v3.3.0＝v2「端末間同期（クラウド保存）」対応＝APP-V2-PP-PRIVACY / APP-CONSENT-TOS17。第17条第2項ただし書き新設・第17条第3項を実装整合。プライバシーポリシー v4.0 と一体で発効）
...
- 文中の波括弧表記は公開時に実値へ置き換える placeholder。**v3.2.1 で {運営者名} のみ「きずなbaton運営事務局」に確定**。**{サービス公開日}＝施行日は附則のとおり 2026-05-27（β施行日）に確定済み**（本注記は旧「デプロイ当日に確定」表記と附則の矛盾を解消＝施行前の未確定状態の名残を是正）。なお**本番一般公開日をβ施行日と別に定めるか**はフルレビューで確定する論点として残す（LEG-42c）。その他の placeholder は v3.2.2 で確定済み。**個人運営者の住所は本規約本文に直接記載しない構成** とし、必要に応じて別途特定商取引法に基づく表記等で開示する想定。
...
- v3.3.0 (2026-07-07): **v2「端末間同期（クラウド保存）」対応（APP-V2-PP-PRIVACY / APP-CONSENT-TOS17・プライバシーポリシー v4.0 と一体で発効）**。(1) 第17条第2項にただし書きを新設＝本人が明示的な有効化操作（アカウント作成＋「クラウド保存」有効化）を行った場合にのみ開始される取扱いの変更は、本サービス内の継続的な告知＋有効化操作時の個別同意をもって同項の30日前事前通知に代替できることとし、有効化しない利用者には当該変更が生じない旨を明記（プライバシーポリシー第17条第2項と同旨・完全オプトイン）。(2) **第17条第3項＝同意ゲート矛盾の解消（オーナー決定 案(a)）**。実装（本人向け同意モーダルで非同意の場合、利用不可＝完全ブロッキング）に条文を整合させ、旧「変更前の機能範囲で継続利用するか退会」を「再同意しない場合は当該バージョン以降を利用不可＝退会・データエクスポートは可能／改定前バージョンの機能のみでの継続利用は提供しない」へ改める。個人開発スケールで旧版並行維持（案(b)）は過剰と判断し案(a)を採用。本項変更は**民法第548条の4（定型約款の変更）**により、変更の必要性・内容の相当性・本項の定めの有無等に照らし合理的なものとして、本サービス内告知等により周知して行う（弁護士フルレビューは emergencyMode 実装ゲートに限定＝本改訂の前提外・オーナー判断改訂）。(3) 版数を v3.2.2→v3.3.0 に更新（`TERMS_VERSION` v3.3.0・`CONSENT_KEY:v25→:v26` で既存利用者へ再同意）。
- v3.3.1 (2026-07-08): **クラウド招待解禁に伴う招待上限表記の実装整合 patch（APP-V2-INVITE-RELEASE・プライバシーポリシー v5.0 と一体で発効）**。2026-07-08 の外部レビュー（Fable A-3）で、第6条第5項第2号「同時保留中5件」が実装（`checkIssueAllowed`＝保留中＋受諾済みの合計5件で制限）より緩い表示であることが判明したため、条文を実装準拠「保留中の招待と受諾済みの家族メンバーはあわせて5件（5人）を上限」へ訂正。同レビューで判明した第6条第5項第4号（受諾時の本人へ即時通知〔メールおよびアプリ内通知〕）・第5条第1項（電子メールアドレスの確認手続き）の未実装は、条文を変えず実装追加（受諾通知メール＝APP-INVITE-ACCEPT-NOTIFY／サインアップ時確認メール＝APP-AUTH-EMAIL-VERIFY）で履行する。民法第548条の4に基づく定型約款の軽微な変更として本サービス内告知等により周知（`TERMS_VERSION` v3.3.1・`CONSENT_KEY:v26→:v27`〔PP v5.0 の material change 由来〕の再同意に同乗）。
