...
test('cleanup: selectExpiredInviteIdsToDelete は revoked/期限切れの保持期間超過のみ選ぶ', () => {
  const R = INVITE_CLEANUP_RETENTION_MS; // 30日
  const now = 1000 * R; // 十分大きな現在時刻
  const docs = [
    { id: 'a', data: { status: 'revoked', revokedAtMs: now - R - 1 } }, // 取消30日超 → 削除
    { id: 'b', data: { status: 'revoked', revokedAtMs: now - 1 } }, // 取消直後 → 残す
    { id: 'c', data: { status: 'pending', expiresAtMs: now - R - 1 } }, // 期限切れ30日超 → 削除
    { id: 'd', data: { status: 'pending', expiresAtMs: now + 1 } }, // 未期限 → 残す
    { id: 'e', data: { status: 'pending', expiresAtMs: now - 10 } }, // 期限切れだが30日未満 → 残す
    { id: 'f', data: { status: 'accepted', acceptedAtMs: now - 10 * R } }, // 受諾済み → 対象外
    { id: 'g', data: { status: 'revoked' } }, // revokedAtMs/expiresAtMs 欠落 → anchor null → 残す
    { id: 'h', data: { status: 'revoked', expiresAtMs: now - R - 1 } }, // revokedAtMs 欠落→expiresAtMs で代替・30日超 → 削除
  ];
  const ids = inviteLib.selectExpiredInviteIdsToDelete(docs, now, R).sort();
  assert.deepEqual(ids, ['a', 'c', 'h']);
});
test('cleanup: 空・不正入力でも落ちない', () => {
  assert.deepEqual(inviteLib.selectExpiredInviteIdsToDelete(null, 0, 1), []);
  assert.deepEqual(inviteLib.selectExpiredInviteIdsToDelete([{}, { id: 'x' }], 0, 1), []);
});
...
// v133(APP-PAID-ANNUAL-VIEW): 1年の見取り図の案内カード経由を独立した導線として計上する
test('recordPaywallView: annual_summary は独立した source として計上される', async () => {
  await signInOwner();
  await call('recordPaywallView')({ source: 'annual_summary' });
  const doc1 = await adminGet('metrics/paywall');
  assert.equal(doc1.bySource.annual_summary, 1);
  assert.equal(doc1.bySource.settings, undefined);
});
