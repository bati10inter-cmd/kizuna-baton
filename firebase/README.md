...
5. **O5 再deploy（stage-2b 変更分・未反映）**＝`firebase deploy --only functions,firestore:rules`。stage-2b で rules（shares フィールドベース化・contracts/assets の get/list 分離）と functions（acceptInvite ownerName 転写・招待リンク実URL `APP_INVITE_BASE_URL`・inviterName 修正）が変わっており本番未反映。**deploy 時に selected＋array-contains の複合インデックス要否を確認**（エミュレータでは要求されない。要求時はエラーメッセージ内リンクから作成）。
